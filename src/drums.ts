...
export function detectDrums(signal: Signal): DrumHit[] {
  ...
  for (let frame = 1; frame < signal.values.length / STRIDE; frame++) {
    const p = frame * STRIDE,
      time = frame / signal.rate;
    ...
    for (let b = 0; b < 5; b++) {
      ...
      if (
        bands[b] < 0.16 ||
        bands[b] < strongest * 0.35 ||
        rise < 0.1 ||
        attack < 0.035 ||
        time - last[b] < refractory
      )
        continue;
      ...
      hits.push({
        ...
        seed: (Math.round(time * 1000) * 2654435761 + b * 1013904223) >>> 0,
      });
    }
  }
...
export function recentHits(hits: DrumHit[], time: number, tail = 0.32, limit = 20) {
  ...
  for (let i = lo - 1; i >= 0 && hits[i].time > time - tail && result.length < limit; i--)
    result.push(hits[i]);
  return result;
}
