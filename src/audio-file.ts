...
export async function identifyAudio(file: Blob): Promise<string | undefined> {
  ...
  if (text.startsWith('ID3') || (bytes[0] === 255 && (bytes[1] & 0xe0) === 0xe0 && (bytes[1] & 0x06) !== 0)) return 'audio/mpeg';
...
export function testSound(rhythm = false): File {
  const rate = 44100, seconds = rhythm ? 24 : 16, count = rate * seconds;
  ...
  for (let i = 0; i < count; i++) {
    const time = i / rate, phase = time % 4, frequency = [90, 800, 5000, 0][Math.floor(time / 4)];
    const envelope = Math.min(1, phase * 30, (4 - phase) * 30) * (0.55 + 0.45 * Math.cos(time * Math.PI * 4));
    let sample = Math.sin(2 * Math.PI * frequency * time) * envelope * 0.22;
    if (rhythm) {
      const beat = time % 0.4, eighth = time % 0.2, step = Math.floor(time / 0.2);
      const note = [60, 64, 67, 71, 69, 67, 64, 62, 57, 60, 64, 67, 65, 64, 60, 59][step % 16];
      const f = 440 * 2 ** ((note - 69) / 12);
      const melody = (Math.sin(time * f * Math.PI * 2) + Math.sin(time * f * Math.PI * 4) * 0.25) * Math.exp(-eighth * 18);
      const bass = Math.sin(time * [65.406, 55, 43.654, 48.999][Math.floor(time / 3.2) % 4] * Math.PI * 2) * Math.exp(-beat * 9);
      const kick = Math.sin(2 * Math.PI * (45 * beat + 10 * (1 - Math.exp(-beat * 30)))) * Math.exp(-beat * 30);
      const hiss = (Math.sin(i * 12.9898) * 43758.5453 % 1) * Math.exp(-eighth * 110);
      sample = (melody * 0.12 + bass * 0.15 + kick * 0.12 + hiss * 0.055) * Math.min(1, time * 20, Math.max(0, 22.4 - time) * 3);
    }
  ...
  return new File([buffer], rhythm ? '잉크 리듬 — 오리지널 샘플.wav' : '진단 사운드.wav', { type: '' });
}
