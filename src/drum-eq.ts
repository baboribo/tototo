...
export function channelLevels(signal: Signal, c: Channel) {
  ...
  const count = s.values.length / s.bins,
    out = new Float32Array(count),
    weights = new Float32Array(s.bins);
  for (let b = 0; b < s.bins; b++) {
    const low = 20 * (s.maxHz / 20) ** (b / s.bins),
      high = 20 * (s.maxHz / 20) ** ((b + 1) / s.bins);
...
export function detectChannels(signal: Signal, channels: Channel[]) {
  const hits: DrumHit[] = [],
    levels = channels.map((c) => channelLevels(signal, c));
  channels.forEach((c, index) => {
    if (!c.enabled) return;
    let baseline = 0,
      last = -Infinity;
    levels[index].forEach((value, f) => {
      const time = f / signal.rate,
        rise = value - baseline,
        attack = value - (levels[index][f - 1] ?? 0);
      baseline += (value - baseline) * 0.12;
      if (value < c.threshold || rise < 0.1 || attack < c.attack || time - last < c.gap / 1000)
        return;
      last = time;
      hits.push({
        time,
        kind: c.kind,
        strength: Math.min(1, rise * 2.5),
        seed:
          (Math.round(time * 1000) * 2654435761 +
            ['kick', 'tom', 'snare', 'hat', 'cymbal'].indexOf(c.kind) * 1013904223) >>>
          0,
      });
    });
  });
