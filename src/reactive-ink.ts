...
export function peakFeature(signal: Signal | undefined, time: number, window: number) {
  ...
  const last = Math.min(
    Math.floor(time * signal.rate),
    Math.ceil(signal.duration * signal.rate) - 1,
  );
...
export function perimeter(
  drums: Signal | undefined,
  other: Signal | undefined,
  time: number,
  gain: number,
  hold: number,
  source: PerimeterSource = 'both',
  impact = 1,
) {
  const bars: InkBar[] = [];
  ...
  for (let slot = 0; slot < 12; slot++) {
    ...
    const attack = clamp(
      Math.max(sample.onset * sample.bands[band], drum.onset * drum.bands[band]) * gain * impact,
    );
    ...
    for (let i = 0; i < length; i++) {
      ...
      const side = Math.floor(distance / 86),
        p = distance % 86;
      const [x, y] =
        side === 0
          ? [117 + p, 77]
...
export function drumInk(hits: DrumHit[], time: number, gain: number, hold: number) {
  ...
  for (const hit of recentHits(hits, time, travel + Math.max(hold, 0.15), Infinity)) {
    const age = time - hit.time,
      strength = clamp(hit.strength * gain);
    ...
    if (age >= travel && age < travel + Math.max(hold, 0.1))
      eraser = Math.max(eraser, Math.min(2, height));
    for (let dx = -halfWidth; dx <= halfWidth; dx++) {
      ...
      if (!previous || previous.height < h)
        columns.set(px, { x: px, height: h, tone: halfWidth > 2 ? 2 : 3, phase: dx + halfWidth });
    }
  }
