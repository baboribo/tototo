...
export class DrumPanel {
  ...
  draw(time:number){
    this.lastTime=time;const ctx=this.canvas.getContext('2d')!,w=960,h=260,c=this.channels[this.selected],x=(f:number)=>Math.log(f/20)/Math.log(1000)*w;
    ctx.fillStyle='#131b19';ctx.fillRect(0,0,w,h);ctx.font='12px monospace';
    for(const f of [20,50,100,200,500,1000,2000,5000,10000,20000]){ctx.strokeStyle='#30413b';ctx.beginPath();ctx.moveTo(x(f),0);ctx.lineTo(x(f),h-24);ctx.stroke();ctx.fillStyle='#9aaea5';ctx.fillText(f>=1000?`${f/1000}k`:String(f),Math.min(w-40,x(f)+4),h-6);}
    ctx.fillStyle=colors[this.selected]+'22';ctx.fillRect(x(c.low),0,x(c.high)-x(c.low),h-24);
    ...
    if(s&&time>=0&&time<this.signal!.duration){ctx.strokeStyle='#d7e7d1';ctx.beginPath();for(let b=0;b<s.bins;b++){
      const power=s.values[frame*s.bins+b]??0,db=10*Math.log10(Math.max(1e-8,power));const px=x(20*(s.maxHz/20)**((b+0.5)/s.bins)),y=232-Math.max(0,Math.min(1,(db+70)/70))*200;b?ctx.lineTo(px,y):ctx.moveTo(px,y);
    }ctx.stroke();}
    ctx.strokeStyle=colors[this.selected];ctx.lineWidth=2;for(const f of [c.low,c.high]){ctx.beginPath();ctx.moveTo(x(f),0);ctx.lineTo(x(f),h-24);ctx.stroke();}ctx.lineWidth=1;
    ...
    const lc=this.thresholdCanvas.getContext('2d')!,lh=180,level=this.levels[this.selected],rate=this.signal?.rate??50;
    const y=(v:number)=>lh*(1-Math.max(0,Math.min(1.6,v))/1.6);
    lc.fillStyle='#111a17';lc.fillRect(0,0,w,lh);lc.font='12px monospace';
    for(const v of [0.4,0.8,1.2,1.6]){lc.strokeStyle='#2d4137';lc.beginPath();lc.moveTo(0,y(v));lc.lineTo(w,y(v));lc.stroke();lc.fillStyle='#8ca294';lc.fillText(v.toFixed(1),4,Math.max(12,y(v)-3));}
    lc.strokeStyle=colors[this.selected];lc.beginPath();
    for(let px=0;px<w;px++){const t=time-3+px/(w-1)*3,v=t>=0?(level?.[Math.floor(t*rate)]??0):0;px?lc.lineTo(px,y(v)):lc.moveTo(px,y(v));}lc.stroke();
    for(const hit of recentHits(this.hits,time,3,Infinity)){if(hit.kind!==c.kind)continue;lc.fillStyle='#f2f6cf';lc.beginPath();lc.arc((hit.time-time+3)/3*w,y(level?.[Math.floor(hit.time*rate)]??0),3,0,Math.PI*2);lc.fill();}
    const current=time>=0&&time<(this.signal?.duration??0)?(level?.[frame]??0):0;
    lc.fillStyle=current>=c.threshold?'#e7eeb5':'#9eb5a8';lc.fillText(`현재 ${current.toFixed(2)} / ${current>=c.threshold?'세기 조건 충족':'기준 미만'}${c.enabled?'':' / 채널 꺼짐'}`,w-300,16);
