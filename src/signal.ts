...
export function analyse(samples: Float32Array, sampleRate: number, spectral = false): Signal {
  const n = 2048,
    count = Math.ceil((samples.length / sampleRate) * RATE);
  const values = new Float32Array(count * STRIDE);
  const bins = 128,
    maxHz = Math.min(20000, sampleRate / 2);
  ...
  for (let frame = 0; frame < count; frame++) {
    ...
    for (let length = 2; length <= n; length <<= 1) {
      ...
      for (let start = 0; start < n; start += length) {
        ...
        for (let j = 0; j < length / 2; j++) {
          const a = start + j,
            b = a + length / 2;
          const tr = real[b] * ur - imaginary[b] * ui,
            ti = real[b] * ui + imaginary[b] * ur;
          real[b] = real[a] - tr;
    ...
    if (spectrum)
      for (let i = 1; i < n / 2; i++) {
        ...
        const bin = Math.min(
          bins - 1,
          Math.floor((Math.log(hz / 20) / Math.log(maxHz / 20)) * bins),
        );
    ...
    for (let band = 0; band < 5; band++) {
      let power = 0;
      for (
        let i = Math.max(1, Math.ceil((edges[band] * n) / sampleRate));
        i < Math.min(n / 2, Math.ceil((edges[band + 1] * n) / sampleRate));
        i++
      )
        power += real[i] ** 2 + imaginary[i] ** 2;
  ...
  const levels = Array.from({ length: count }, (_, i) => values[i * STRIDE + 5]).sort(
    (a, b) => a - b,
  );
  const reference = Math.max(0.015, levels[Math.floor(count * 0.95)] ?? 0);
  if (spectrum)
    for (let i = 0; i < spectrum.values.length; i++) spectrum.values[i] /= (reference * 0.58) ** 2;
  let baseline = 0,
    envelope = 0;
  for (let frame = 0; frame < count; frame++) {
  ...
  return {
    ...
    rate: RATE,
    tempo: detectTempo(values, RATE, STRIDE),
    spectrum,
  };
}
...
export function at(signal: Signal | undefined, seconds: number): Feature {
  ...
  const index =
    Math.min(Math.floor(seconds * signal.rate), signal.values.length / STRIDE - 1) * STRIDE;
  return {
    bands: Array.from(signal.values.subarray(index, index + 5)),
    level: signal.values[index + 5],
    onset: signal.values[index + 6],
  };
}
