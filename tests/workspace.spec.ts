...
test('preload prepares the full song, seeks deterministically and rebuilds after settings change',async({page})=>{
  const errors:string[]=[];page.on('pageerror',error=>errors.push(error.message));
  await page.goto('/');
  await expect(page.locator('#drum-spectrum')).toBeAttached();
  await page.locator('#drum-file').setInputFiles({name:'drums.wav',mimeType:'audio/wav',buffer:wavTone(90,4)});
  await page.locator('#other-file').setInputFiles({name:'other.wav',mimeType:'audio/wav',buffer:wavTone(440,4)});
  await page.locator('#apply-stems').click();
  ...
  const pixels=await page.locator('#visualizer').evaluate((c:HTMLCanvasElement)=>c.toDataURL());
  await page.locator('#scrubber').fill('3');
  await page.locator('#scrubber').fill('1');
  expect(await page.locator('#visualizer').evaluate((c:HTMLCanvasElement)=>c.toDataURL())).toBe(pixels);
  await page.getByRole('tab',{name:'화면',exact:true}).click();
  await page.locator('#bpm').fill('120');
  await expect(page.locator('#tile-mode-readout')).toContainText('준비 완료',{timeout:30000});
  await expect(page.locator('#toggle')).toBeEnabled();
  ...
  await page.locator('#toggle').click();
  await expect.poll(()=>page.locator('#audio').evaluate((a:HTMLAudioElement)=>a.currentTime)).toBeGreaterThan(1.1);
  await page.locator('#toggle').click();
  await page.getByRole('tab',{name:'오디오',exact:true}).click();
  await page.locator('#apply-stems').click();
  await page.locator('#choose-live').click();
