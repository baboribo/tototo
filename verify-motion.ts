...
const rate = 22050;
const tone = (hz: number, duration = 2) => Float32Array.from({ length: rate * duration }, (_, i) => 0.18 * Math.sin(i / rate * Math.PI * 2 * hz));
for (const [hz, expected] of [[90, 0], [300, 1], [900, 2], [5000, 3], [9500, 4]]) {
  const signal = analyse(tone(hz), rate), feature = at(signal, 1);
...
assert(silent.values.every(v => v === 0), 'silence must remain zero');
...
const a = planFrame(signal, 0.6, 1, {impact:0}).tiles, b = planFrame(signal, 0.7, 1, {impact:0}).tiles;
...
for (const expected of [80, 100, 120, 127, 150, 180]) {
  ...
  for (let t = offset; t < 24; t += period) {
    ...
    for(let j = 0; j < 4; j++) if ((i+j)*STRIDE+6 < values.length) { values[(i+j)*STRIDE+6] = Math.exp(-j); values[(i+j)*STRIDE] = Math.exp(-j); values[(i+j)*STRIDE+5] = Math.exp(-j); }
  }
...
const fake = { values: new Float32Array(50*4*STRIDE), duration:4, rate:50, tempo:{ bpm:120,offset:0.23,confidence:1 } };
for(let i=0;i<200;i++) { fake.values[i*STRIDE]=0.8; fake.values[i*STRIDE+2]=0.65; fake.values[i*STRIDE+5]=0.8; }
const flash = planFrame(fake,1.3,1,{impact:1}), off = planFrame(fake,1.3,1,{impact:0});
